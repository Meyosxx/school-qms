...
function Login() {
  ...
  useEffect(() => {
    ...
    axios.get(apiUrl('/api/settings/carousel'))
  ...
  return (
    <div className={`login-split-container ${isSwapping ? 'login-swap-animating' : ''}`}>
      
      {/* LEFT SIDE: CAROUSEL WITH CONTROLS */}
      {/* We use the secondary color as the background padding */}
      <div className="login-left-carousel" style={{ backgroundColor: settings?.secondary_color || '#1F4E79' }}>
      ...
      <div 
        ...
        style={{ 
          ...
          background: `linear-gradient(135deg, #ffffff 40%, ${settings?.secondary_color || '#1F4E79'} 100%)` 
        }}
      >
        <div className="login-card">
          
          {settings?.logo_path ? (
            <img src={uploadUrl(settings.logo_path)} alt="School Logo" className="login-logo" />
          ) : (
          ...
          <form onSubmit={handleLogin}>
            ...
            <button type="submit" className="login-btn" style={{ backgroundColor: settings?.primary_color || '#C63D3D' }}>
              LOGIN
            </button>
